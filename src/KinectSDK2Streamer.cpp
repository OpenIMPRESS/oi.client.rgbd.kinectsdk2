	...
	namespace core {
		namespace rgbd {
			...
			bool KinectDSK2Streamer::OpenDevice() {
				...
				audio_thread = new std::thread(&KinectDSK2Streamer::GetAudio, this);

				return true;
			}

			void KinectDSK2Streamer::GetAudio() {
				_audio_running = true;
			...
			int KinectDSK2Streamer::SendFrame() {
				...
				HRESULT hres = S_OK;
				/*
				const size_t audio_buffer_size = 6400;
				float audio_buffer[audio_buffer_size];
				int audio_buffer_loc = GetAudioSamples(audioReader, &(audio_buffer[0]), audio_buffer_size);
				if (audio_buffer_loc > 0) {
					res += _SendAudioFrame(sequence, audio_buffer, audio_buffer_loc, 16000, 1, timestamp);
				}*/
			...
			bool KinectDSK2Streamer::supports_body() {
				return true;
			}

			bool KinectDSK2Streamer::supports_bidx() {
				return true;
			}

			bool KinectDSK2Streamer::supports_hd() {
				return true;
			}

			int KinectDSK2Streamer::GetFrame(IMultiSourceFrame * frame, IColorFrame ** res) {
			...
			int KinectDSK2Streamer::GetFrame(IMultiSourceFrame * frame, IDepthFrame ** res) {
			...
			int KinectDSK2Streamer::GetFrame(IMultiSourceFrame * frame, IBodyFrame ** res) {
			...
			int KinectDSK2Streamer::GetFrame(IMultiSourceFrame * frame, IBodyIndexFrame ** res) {
			...
			int KinectDSK2Streamer::GetAudioSamples(IAudioBeamFrameReader * ar, float * dst, size_t max) {
